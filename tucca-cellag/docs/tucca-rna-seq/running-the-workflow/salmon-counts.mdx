---
title: Getting Salmon Pseudocounts
---
...
<Tabs>
  <TabItem value="tufts-hpc" label="Tufts HPC Users" default>
    ...
    Unlike the installation process, when running the workflow, it is best to
    run on a login node of the Tufts HPC run rather than a compute node.

    After login you should already be on a Login Node of the cluster
    (`login-prod-0x`) and in your Home Directory (`~` or
    `/cluster/home/your_utln`). Your terminal should look like this:
