...
description: Configure and interpret GO, KEGG, WikiPathways, MSigDB, SPIA, and Harmonizome functional enrichment in the tucca-rna-seq workflow.
---
...
The `tucca-rna-seq` workflow provides functional enrichment
...
The workflow uses `clusterProfiler` for enrichment analysis:
...
:::note[Interactive Visualization Through Shiny Apps]
While the workflow doesn't generate static image files, it creates
data files (RDS/TSV) that power interactive Shiny applications.
...
- **Integration**: Provides the correct annotation data for
  enrichment analysis
...
[Running Guide](/tucca-rna-seq/running/).
...
### What the Workflow Provides
✅ **Data objects** for all enrichment analyses  
...
expression changes. The workflow generates data objects and
