...
description: How to set up config.yaml, samples.tsv, units.tsv, and execution profiles to configure the tucca-rna-seq workflow for your analysis.
---
...
Make sure you've completed the [Data Collection Template](/tucca-rna-seq/data-collection/data-collection-how/) 
...
:::note[Data Source Configuration]
...
:::caution[Recommended: Use .env File]
**We strongly recommend using a `.env` file instead of setting API keys in
`config.yaml`.** The `.env` file is git-ignored and prevents accidental commits
...
1. **Environment variable** `NCBI_API_KEY` (highest priority)
2. **`.env` file** (if `NCBI_API_KEY` is set in the file)
3. **Config file** `config["api_keys"]["ncbi"]` (fallback, deprecated)

This ensures your API keys remain secure and are never committed to version
control.
:::
...
**After obtaining your key, add it to your `.env` file** (not in `config.yaml`)
to keep it secure.
:::
...
Configure functional enrichment analysis including GO, KEGG,
...
:::caution[KEGG API Usage Restrictions]
...
workflows, provides a list of all possible flags and options that
...
for all official Snakemake plugins. These plugins extend
...
<Tabs>
  <TabItem label="SLURM-managed HPC Cluster">
  ...
  <TabItem label="CI via GitHub Actions (Conda-only)">
  ...
  <TabItem label="Google Cloud Batch and Storage">
  {/* TODO */}
  *Coming Soon...*
  </TabItem>
</Tabs>
