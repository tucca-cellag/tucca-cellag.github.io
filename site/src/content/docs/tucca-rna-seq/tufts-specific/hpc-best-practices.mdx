...
integrate it into your HPC workflow. This makes loading your custom
...
- `-p`: Partition (`batch` by default; also `gpu` or `preempt`)
...
1.  **Start a long-running `screen` or `tmux` session.**
    A terminal multiplexer keeps your workflow from being
